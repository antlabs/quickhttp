...
func (u *URI) isHTTPS() bool {
	return bytes.Equal(u.scheme, bytesHTTPS)
}

func (u *URI) isHTTP() bool {
	return len(u.scheme) == 0 || bytes.Equal(u.scheme, bytesHTTP)
}
...
func (u *URI) parse(host, uri []byte, isTLS bool) error {
	...
	if len(host) == 0 || bytes.Contains(uri, bytesColonSlashSlash) {
...
func normalizePath(dst, src []byte) []byte {
	...
	bSize := len(b)
	for {
		n := bytes.Index(b, bytesSlashSlash)
		if n < 0 {
			break
		}
		b = b[n:]
	...
	b = dst
	for {
		n := bytes.Index(b, bytesSlashDotSlash)
		if n < 0 {
			break
		}
		nn := n + len(bytesSlashDotSlash) - 1
		copy(b[n:], b[nn:])
		b = b[:len(b)-nn+n]
	}
	...
	// remove /foo/../ parts
	for {
		n := bytes.Index(b, bytesSlashDotDotSlash)
		if n < 0 {
			break
		}
		...
		n += len(bytesSlashDotDotSlash) - 1
		copy(b[nn:], b[n:])
		b = b[:len(b)-n+nn]
	}

	// remove trailing /foo/..
	n := bytes.LastIndex(b, bytesSlashDotDot)
	if n >= 0 && n+len(bytesSlashDotDot) == len(b) {
		nn := bytes.LastIndexByte(b[:n], '/')
		if nn < 0 {
			return append(dst[:0], bytesSlash...)
		}
		b = b[:nn+1]
	}
	...
	if filepath.Separator == '\\' {
		// remove \.\ parts
		for {
			n := bytes.Index(b, bytesBackSlashDotBackSlash)
			if n < 0 {
				break
			}
			nn := n + len(bytesSlashDotSlash) - 1
		...
		// remove /foo/..\ parts
		for {
			n := bytes.Index(b, bytesSlashDotDotBackSlash)
			if n < 0 {
				break
			}
			nn := bytes.LastIndexByte(b[:n], '/')
			...
			nn++
			n += len(bytesSlashDotDotBackSlash)
			copy(b[nn:], b[n:])
			b = b[:len(b)-n+nn]
		}
		...
		// remove /foo\..\ parts
		for {
			n := bytes.Index(b, bytesBackSlashDotDotBackSlash)
			if n < 0 {
				break
			}
			nn := bytes.LastIndexByte(b[:n], '/')
			...
			n += len(bytesBackSlashDotDotBackSlash) - 1
			copy(b[nn:], b[n:])
			b = b[:len(b)-n+nn]
		}

		// remove trailing \foo\..
		n := bytes.LastIndex(b, bytesBackSlashDotDot)
		if n >= 0 && n+len(bytesSlashDotDot) == len(b) {
			nn := bytes.LastIndexByte(b[:n], '/')
			if nn < 0 {
				return append(dst[:0], bytesSlash...)
...
func (u *URI) updateBytes(newURI, buf []byte) []byte {
	...
	n := bytes.Index(newURI, bytesSlashSlash)
	if n >= 0 {
		// absolute uri
		var b [32]byte
		schemeOriginal := b[:0]
...
func (u *URI) appendSchemeHost(dst []byte) []byte {
	...
	dst = append(dst, bytesColonSlashSlash...)
...
func splitHostURI(host, uri []byte) ([]byte, []byte, []byte) {
	n := bytes.Index(uri, bytesSlashSlash)
	if n < 0 {
		return bytesHTTP, host, uri
	}
	scheme := uri[:n]
